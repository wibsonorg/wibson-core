...
contract("DataOrder", accounts => {
  ...
  describe("addDataResponse", function() {
    it("can not add a data response if order is closed", async function() {
      ...
      try {
        await order.addDataResponse(seller, notary, dataHash, signature, {
          from: owner
        });
        assert.fail();
      } catch (error) {
    ...
    it("can not add a data response if seller is 0x0", async function() {
      try {
        ...
        await order.addDataResponse(0x0, notary, dataHash, sig, {
          from: owner
        });
        assert.fail();
      } catch (error) {
    ...
    it("can not add a data response if seller has same address as Data Order", async function() {
      try {
        const sig = signMessage(
          [order.address, order.address, notary, dataHash],
          seller
        );
        await order.addDataResponse(order.address, notary, dataHash, sig, {
          from: owner
        });
        assert.fail();
      } catch (error) {
    ...
    it("can not add a data response if notary is 0x0", async function() {
      try {
        ...
        await order.addDataResponse(seller, 0x0, dataHash, sig, {
          from: owner
        });
        assert.fail();
      } catch (error) {
    ...
    it("can not add a data response if notary has same address as Data Order", async function() {
      try {
        const sig = signMessage(
          [order.address, seller, order.address, dataHash],
          seller
        );
        await order.addDataResponse(seller, order.address, dataHash, sig, {
          from: owner
        });
        assert.fail();
      } catch (error) {
    ...
    it("can not add a data response if notary was not added to Data Order", async function() {
      try {
        const sig = signMessage(
          [order.address, seller, inexistentNotary, dataHash],
          seller
        );
        await order.addDataResponse(seller, inexistentNotary, dataHash, sig, {
          from: owner
        });
        assert.fail();
      } catch (error) {
    ...
    it("should add a data response even if dataHash is empty", async function() {
      try {
        ...
        await order.addDataResponse(seller, notary, "", sig, { from: owner });
        const sellerWasAdded = await order.hasSellerBeenAccepted(seller);
        assert(sellerWasAdded, "Seller was not added correctly");
      } catch (error) {
        assert.fail();
    ...
    it("should check if seller has been accepted", async function() {
      ...
      assert.isNotOk(
        sellerNotExists,
        "Seller that not exists has been accepted"
      );
    });
