...
contract.only('DataExchange', (accounts) => {
  ...
  it('should do complete flow', async () => {
    ...
    // tx = await batPay.transfer(id, 1, PAYDATA.toArray(), 0, payDataHash, lock, metadata, { from: BUYER });
    tx = await batPay.transfer(id, 1, PAYDATA_PADDED, 0, payDataHash, lock, metadata, { from: BUYER });
